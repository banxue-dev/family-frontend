...
layui.define(['laytpl', 'layer'], function(exports){
  ...
  view.req = function(options){
    ...
    if(request.tokenName){
      ...
      : options.data;
       //自动给 Request  传入 orgCode,如果参数里面没有orgcode的情况下才会传入
       if(!options.data[request.orgCodeName]){
        options.data[request.orgCodeName] = request.orgCodeName in sendData
          ?  options.data[request.orgCodeName]
        : (layui.data(setter.tableName)[request.orgCodeName] || '');
       }

       //自动给 Request Headers 传入 userId
      options.headers[request.userIdName] = request.userIdName in options.headers 
        ?  options.headers[request.userIdName]
      : (layui.data(setter.tableName)[request.userIdName] || '');
    ...
    return $.ajax($.extend({
      ...
      ,success: function(res){
        ...
        //其它异常
          ...
          view.error((res[response.msgName] || '返回状态码异常'));
        }
  ...
  Class.prototype.getParams = function(views, params){
    return "1";
  };
